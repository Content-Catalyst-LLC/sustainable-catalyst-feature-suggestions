...
DISCOVERY_BACKEND="$ROOT_DIR/backend/app/support_discovery.py"
DISCOVERY_TEST="$ROOT_DIR/backend/tests/test_support_discovery.py"
...
RELEASE_MANIFEST="$ROOT_DIR/release-manifest-v5.2.9.json"
...
for path in "$MAIN" "$KB" "$SUPPORT" "$INTEGRITY" "$INTEGRITY_CSS" "$KB_CSS" "$SUPPORT_CSS" "$MANIFEST" "$RELEASE_MANIFEST" "$BACKEND_MODULE" "$BACKEND_TEST" "$DISCOVERY" "$DISCOVERY_BACKEND" "$DISCOVERY_TEST" "$ROOT_DIR/RELEASE_NOTES_5.2.9.md" "$ROOT_DIR/docs/support-discovery-navigation-search-v5.2.9.md"; do
...
grep -Fq 'Version: 5.2.9' "$MAIN" || fail "Plugin version header mismatch."
grep -Fq "const VERSION = '5.2.9';" "$MAIN" || fail "Runtime version mismatch."
...
grep -Fq 'final class SCFS_Support_Discovery' "$DISCOVERY" || fail "Support discovery class missing."
grep -Fq "const VERSION = '5.2.9';" "$DISCOVERY" || fail "Support discovery version mismatch."
grep -Fq '/knowledge-base/discovery/search' "$DISCOVERY" || fail "WordPress discovery search route missing."
grep -Fq '/v1/support-discovery/search' "$ROOT_DIR/backend/app/main.py" || fail "FastAPI discovery search endpoint missing."
...
assert manifest['version'] == '5.2.9'
assert manifest['release_name'] == 'Support Discovery, Navigation, and Search Quality'
...
assert release['version'] == '5.2.9'
assert release['compatibility']['support_article_permalink_base'] == '/support/guides/'
assert release['support_discovery']['schema'] == 'scfs-support-discovery/1.0'
assert release['support_discovery']['personal_data_stored'] is False
