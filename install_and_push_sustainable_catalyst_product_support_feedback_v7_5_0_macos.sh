...
set -Eeuo pipefail
VERSION="7.5.0"
INSTALLER_REVISION="V7_5_0_RELEASE_INTELLIGENCE_COPY_CONTROLS"
RELEASE_TITLE="Release Intelligence and Console Copy Controls"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v7_5_0.sh"
REPO_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.5.0-repository.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v7.5.0-release-bundle.zip"
SUMS_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.5.0-artifacts.sha256"
...
[ -n "$ARCHIVE" ] || fail "Place the v7.5.0 repository ZIP or release bundle beside this installer or in ~/Downloads."
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scpsf-v750.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
if [ -n "$REPO_ZIP" ]; then
  ...
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.5.0-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.5.0-repository' -print -quit)"
fi
[ -n "${SOURCE:-}" ] || fail "Could not locate the v7.5.0 repository source."
...
grep -Fq 'VERSION="7.5.0"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "Wrong validator version."
grep -Fq 'Version: 7.5.0' "$SOURCE/wordpress/$PLUGIN_SLUG/$PLUGIN_SLUG.php" || fail "Wrong plugin version."
...
grep -Fq 'MutationObserver' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-console-v7.5.0.js" || fail "Dynamic Release Console initialization is missing."
grep -Fq '[data-console-active="true"]' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.5.0.css" || fail "Stable Release Console screen layout is missing."
...
if [ "${SCPSF_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v7.5.0 Release Intelligence and Console Copy Controls package and preserved the WordPress plugin identity.\n'
  exit 0
fi
...
printf '==> Installing v7.5.0 source with checksum verification\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v7.5.0.\n'
else
  git commit -m "Product Support and Feedback Platform v7.5.0 — $RELEASE_TITLE"
fi
...
push_release_tag
printf '\nSUCCESS: v7.5.0 validated, installed, committed, tagged, and pushed.\n'
