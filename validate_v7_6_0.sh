...
rm -f "$contract_log"
[ "$contract_count" = "279" ] || fail "Expected 279 PHP contracts, got $contract_count."
printf 'PASS - %d PHP contract files executed sequentially\n' "$contract_count"

printf '==> Runtime identity and preserved WordPress slug\n'
php -r '$p=file_get_contents($argv[1]); if(!preg_match("/Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform/",$p)||!preg_match("/Version:[[:space:]]*7\\.6\\.0/",$p)||!preg_match("/const VERSION = '\''7\\.6\\.0'\'';/",$p)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity mismatch."
...
[ "$js_count" = "45" ] || fail "Expected 45 JavaScript files, got $js_count."
...
COUNTS
[ "$json_count" = "237" ] || fail "Expected 237 JSON files, got $json_count."
...
assert current['validation']['php_contract_files'] == 279
assert current['validation']['fastapi_tests'] == 318
assert current['validation']['javascript_files'] == 45
assert current['validation']['css_layers'] == 56
assert current['validation']['json_files'] == 237
...
for key in ('all_products_operational_table','freshness_health','bulk_sync_selected','bulk_sync_all_connected','bulk_clear_errors','integrity_audit','nonsecret_json_export','wp_cli_report','credentials_excluded'):
    assert current['release_operations_admin'][key] is True,key
for key in ('release_operations_admin','release_operations_runtime','release_operations_freshness_health','release_operations_bulk_sync','release_operations_integrity_audit','release_operations_nonsecret_export'):
