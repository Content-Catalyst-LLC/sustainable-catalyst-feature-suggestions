...
if [ ! -f tests/test-v520-support-documentation-library.php ]; then
  echo "ERROR: The v5.2.2 Support Documentation Library contract test is missing."
  exit 1
fi
...
' "$LAYOUT_OUTPUT"
if ! printf '%s
' "$LAYOUT_OUTPUT" | grep -Fq 'v5.2.2 support documentation library contract passed (18 checks).'; then
  echo "ERROR: The v5.2.2 Support Documentation Library contract did not pass all 18 checks."
  exit 1
fi
