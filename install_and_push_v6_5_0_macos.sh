...
VERSION="6.5.0"
INSTALLER_REVISION="V6_5_0_REMOTE_SYNC_SAFE"
RELEASE_TITLE="Secure Evidence, Attachments, and Diagnostic Intake"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v6_5_0.sh"
...
ARCHIVE=""
for candidate in \
  "$SCRIPT_DIR/sustainable-catalyst-feature-suggestions-v6.5.0-repo.zip" \
  "$DOWNLOADS/sustainable-catalyst-feature-suggestions-v6.5.0-repo.zip" \
  "$SCRIPT_DIR/sustainable-catalyst-product-support-feedback-platform-v6.5.0-release-bundle.zip" \
  "$DOWNLOADS/sustainable-catalyst-product-support-feedback-platform-v6.5.0-release-bundle.zip"; do
  if [ -f "$candidate" ]; then
    ARCHIVE="$candidate"
    break
  fi
done
[ -n "$ARCHIVE" ] || fail "Place the v6.5.0 repository ZIP or release bundle beside this installer or in ~/Downloads."

TMP="$(mktemp -d "${TMPDIR:-/tmp}/scfs-v650.XXXXXX")"
cleanup() { rm -rf "$TMP"; }
trap cleanup EXIT
...
REPO_ZIP="$(find "$TMP/archive" -type f -name 'sustainable-catalyst-feature-suggestions-v6.5.0-repo.zip' -print -quit || true)"
if [ -n "$REPO_ZIP" ]; then
  mkdir -p "$TMP/repository"
  unzip -q "$REPO_ZIP" -d "$TMP/repository"
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-feature-suggestions-v6.5.0-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-feature-suggestions-v6.5.0-repository' -print -quit)"
fi

[ -n "${SOURCE:-}" ] || fail "Could not locate the v6.5.0 repository source."
...
[ -f "$SOURCE/wordpress/sustainable-catalyst-feature-suggestions/includes/class-scfs-help-desk-secure-evidence.php" ] || fail "The selected archive is missing the v6.5.0 Secure Evidence implementation."
[ -f "$SOURCE/schemas/scfs-help-desk-secure-evidence-v1.schema.json" ] || fail "The selected archive is missing the v6.5.0 Secure Evidence schema."
grep -Fq 'VERSION="6.5.0"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "The packaged validator is not the v6.5.0 validator."
grep -Fq 'blank EOF lines' "$SOURCE/$VALIDATOR_SCRIPT" || fail "The selected archive does not contain the v6.5.0 whitespace preflight."
grep -Fq 'Version: 6.5.0' "$SOURCE/wordpress/sustainable-catalyst-feature-suggestions/sustainable-catalyst-feature-suggestions.php" || fail "The selected archive does not contain the v6.5.0 plugin source."

if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$SOURCE/$VALIDATOR_SCRIPT"; then
  fail "The packaged validation script requires Bash 4. Use the portable v6.5.0 repaired release."
fi
...
if [ "${SCFS_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v6.5.0 archive and validator.\n'
  exit 0
fi
...
if [ -d "$REPO_DIR/.git" ]; then
  ...
  BACKUP="$DOWNLOADS/sustainable-catalyst-feature-suggestions-before-v6.5.0-${TIMESTAMP}.zip"
  GIT_BUNDLE="$DOWNLOADS/sustainable-catalyst-feature-suggestions-before-v6.5.0-${TIMESTAMP}.bundle"
...
printf '==> Installing v6.5.0 source onto current origin/main\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v6.5.0.\n'
else
  git commit -m "Product Support and Feedback Platform v6.5.0 — $RELEASE_TITLE"
fi
...
printf '\nSUCCESS: v6.5.0 synchronized with current origin/main, validated with %s, committed, tagged, and pushed.\n' "$VALIDATOR_SCRIPT"
