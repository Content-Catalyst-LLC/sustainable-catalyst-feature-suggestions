...
VERSION="5.2.8"
INSTALLER_REVISION="V5_2_8_R1_ARTICLE_INTEGRITY"
RELEASE_NAME="Support Article Content Integrity and Publication Validation"
REPOSITORY="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-feature-suggestions.git"
DOWNLOADS_DIR="${HOME}/Downloads"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/scfs-v528.XXXXXX")"
...
[ -n "$PYTHON_BIN" ] || fail 'Python 3.12 or 3.13 is required for v5.2.8 validation. Install one with: brew install python@3.13'
...
patterns = (
    'sustainable-catalyst-product-support-feedback-platform-v5.2.8-release-bundle*.zip',
    'sustainable-catalyst-feature-suggestions-v5.2.8-repo*.zip',
)
files = []
...
[ -f "$SOURCE_ARCHIVE" ] || fail 'No v5.2.8 repository ZIP or release bundle was found in ~/Downloads.'
printf 'Using release archive: %s\n' "$SOURCE_ARCHIVE"
unzip -q "$SOURCE_ARCHIVE" -d "$STAGE_DIR"

INNER_ZIP="$(find "$STAGE_DIR" -maxdepth 8 -type f -name 'sustainable-catalyst-feature-suggestions-v5.2.8-repo*.zip' -print -quit)"
if [ -n "$INNER_ZIP" ]; then
...
integrity = manifest.get('publication_integrity', {})
assert integrity.get('schema') == 'scfs-support-article-integrity/1.0'
assert integrity.get('automatic_content_changes') is False
assert integrity.get('automatic_publication') is False
PY
...
for required in \
  ...
  tests/test-v528-bootstrap.php \
  tests/test-v528-integrity-engine.php \
  tests/test-v528-admin-and-api.php \
  tests/test-v528-compatibility.php \
  tests/test-v528-release-artifacts.php \
  docs/support-article-content-integrity-v5.2.8.md \
  examples/support-article-integrity-assessment.json \
  RELEASE_NOTES_5.2.8.md \
  release-manifest-v5.2.8.json \
  validate_v5_2_8.sh; do
...
PYTHON_BIN="$VENV_DIR/bin/python" ./validate_v5_2_8.sh
...
printf 'Article Integrity: WordPress Admin → Support & Feedback → Article Integrity\n'
