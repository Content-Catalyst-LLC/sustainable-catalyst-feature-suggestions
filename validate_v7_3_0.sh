...
contract_log="$(mktemp "${TMPDIR:-/tmp}/scpsf-v730-contract.XXXXXX")"
...
php -r '$plugin=file_get_contents($argv[1]);if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform[[:space:]]*$/m",$plugin)){exit(1);}if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Version:[[:space:]]*7\\.3\\.0[[:space:]]*$/m",$plugin)){exit(1);}if(!preg_match("/const VERSION = '\''7\\.3\\.0'\'';/",$plugin)){exit(1);}if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Text Domain:[[:space:]]*sustainable-catalyst-feature-suggestions[[:space:]]*$/m",$plugin)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity or compatibility check failed."
...
printf '==> Release blackboard manifests and schema\n'
"$PYTHON_BIN" - "$ROOT" <<'PYMANIFEST'
from pathlib import Path
import json,sys
r=Path(sys.argv[1])
current=json.loads((r/'feature_suggestions_manifest.json').read_text())
versioned=json.loads((r/'feature_suggestions_manifest-v7.3.0.json').read_text())
historical=json.loads((r/'feature_suggestions_manifest-v7.2.1.json').read_text())
release=json.loads((r/'release-manifest-v7.3.0.json').read_text())
example=json.loads((r/'examples/release-board-v7.3.0.json').read_text())
schema=json.loads((r/'schemas/scfs-release-board-v1.schema.json').read_text())
assert current==versioned
assert current['version']=='7.3.0'
assert current['release_name']=='Release Blackboard Shortcode'
assert current['repository']=='Content-Catalyst-LLC/sustainable-catalyst-product-support-feedback'
board=current['release_board']
assert board['schema']=='scfs-release-board/1.0'
assert board['shortcode']=='sc_release_board'
assert board['layouts']==['blackboard','compact','directory']
for key in ('canonical_registry_source','installed_and_manual_versions_combined','homepage_visibility_governed','inactive_products_hidden_by_default','semantic_list_output','status_text_visible','responsive_without_javascript','cache_safe_asset_versioning'):
...
assert release['version']=='7.3.0'
assert release['release_name']=='Release Blackboard Shortcode'
assert release['compatibility']['database_migration_required'] is False
assert example['schema']==board['schema']
assert example['version']=='7.3.0'
assert schema['properties']['version']['const']=='7.3.0'
print('PASS - release board identity, governance, privacy, accessibility, and compatibility contracts validated')
PYMANIFEST
printf '==> Release blackboard source contract\n'
BOARD_CLASS="$PLUGIN/includes/class-scfs-release-board.php"
BOARD_CSS="$PLUGIN/assets/release-board-v7.3.0.css"
[ -f "$BOARD_CLASS" ] || fail "Missing release board class."
[ -f "$BOARD_CSS" ] || fail "Missing release board stylesheet."
grep -Fq "const VERSION = '7.3.0';" "$BOARD_CLASS" || fail "Release board version mismatch."
...
for attribute in layout context groups products limit show_status show_updated show_links inactive title heading_level; do grep -Fq "'$attribute'" "$BOARD_CLASS" || fail "Shortcode attribute missing: $attribute"; done
...
INSTALLER="$ROOT/install_and_push_sustainable_catalyst_product_support_feedback_v7_3_0_macos.sh"
[ -f "$INSTALLER" ] || fail "Missing v7.3.0 installer."
...
if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$INSTALLER" "$ROOT/validate_v7_3_0.sh"; then fail "Release scripts require Bash 4."; fi
