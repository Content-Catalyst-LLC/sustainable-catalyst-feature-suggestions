...
PORTAL_CSS="$PLUGIN_DIR/assets/help-desk-customer-portal.css"
PORTAL_JS="$PLUGIN_DIR/assets/help-desk-customer-portal.js"
MANIFEST="$ROOT_DIR/feature_suggestions_manifest.json"
RELEASE_MANIFEST="$ROOT_DIR/release-manifest-v6.3.0.json"
SCHEMA="$ROOT_DIR/schemas/scfs-help-desk-customer-portal-v1.schema.json"
EXAMPLE="$ROOT_DIR/examples/help-desk-customer-portal-v6.3.0.json"

for path in \
  "$MAIN" "$FOUNDATION" "$WORKSPACE" "$PORTAL" "$PORTAL_CSS" "$PORTAL_JS" \
  "$MANIFEST" "$RELEASE_MANIFEST" "$SCHEMA" "$EXAMPLE" \
  "$ROOT_DIR/backend/app/help_desk_customer_portal.py" \
  "$ROOT_DIR/backend/tests/test_help_desk_customer_portal.py" \
  "$ROOT_DIR/backend/requirements-validation.txt" \
  "$ROOT_DIR/docs/help-desk-customer-portal-v6.3.0.md" \
  "$ROOT_DIR/install_and_push_v6_3_0_macos.sh"; do
  [ -f "$path" ] || fail "Missing required release file: ${path#$ROOT_DIR/}"
done

grep -Fq 'Version: 6.3.0' "$MAIN" || fail "Plugin version header mismatch."
grep -Fq "const VERSION = '6.3.0';" "$MAIN" || fail "Runtime version mismatch."
...
grep -Fq 'final class SCFS_Help_Desk_Customer_Portal' "$PORTAL" || fail "Customer Portal class missing."
grep -Fq "const SCHEMA = 'scfs-help-desk-customer-portal/1.0';" "$PORTAL" || fail "Customer Portal schema mismatch."
grep -Fq "const SHORTCODE = 'scfs_help_desk_customer_portal';" "$PORTAL" || fail "Customer Portal shortcode missing."
grep -Fq "'raw_access_tokens_stored' => false" "$PORTAL" || fail "Raw token privacy boundary missing."
grep -Fq "'raw_session_secrets_stored' => false" "$PORTAL" || fail "Raw session privacy boundary missing."
grep -Fq "'internal_notes_exposed' => false" "$PORTAL" || fail "Internal-note privacy boundary missing."
grep -Fq "'public_case_list_api' => false" "$PORTAL" || fail "Public case-list boundary missing."
if grep -Fq "'portal_url' => \$this->portal_url(\$secret)" "$PORTAL" | grep -q notification; then
  fail "Raw portal URL must not be persisted in notification metadata."
fi
...
assert manifest['release_name'] == 'Customer Support Portal and Conversations'
assert manifest['compatibility']['rest_namespace'] == 'scfs/v1'
portal = manifest['help_desk_customer_portal']
assert portal['schema'] == 'scfs-help-desk-customer-portal/1.0'
assert portal['db_version'] == '1.2.0'
assert len(portal['additive_tables']) == 5
for key in ('managed_portal_page','token_to_session_exchange','secure_http_only_cookie','clean_url_redirect_after_exchange','participant_visible_messages_only','requester_replies','requester_resolution_actions','bounded_reopen_window','private_satisfaction_feedback','authenticated_customer_rest_api','protected_admin_rest_api','wp_cli','human_review_required'):
    assert portal[key] is True, key
for key in ('raw_access_tokens_stored','raw_session_secrets_stored','public_case_list_api','case_existence_disclosure_without_session','internal_notes_exposed','private_attachment_bytes_exposed','automatic_case_creation','automatic_case_resolution','automatic_feedback_publication'):
    assert portal[key] is False, key
for key in ('identity_authority','attachment_authority','notification_authority'):
    assert portal[key] == 'contact-engagement', key
assert release['version'] == '6.3.0'
assert release['release_name'] == 'Customer Support Portal and Conversations'
...
assert release['compatibility']['additive_portal_schema_activation_required'] is True
assert schema['properties']['version']['const'] == '6.3.0'
assert schema['properties']['schema']['const'] == 'scfs-help-desk-customer-portal/1.0'
assert example['schema'] == 'scfs-help-desk-customer-portal/1.0'
assert example['public_case_list_api'] is False
assert example['privacy']['internal_notes_exposed'] is False
assert example['privacy']['raw_access_token_stored'] is False
print('PASS - release identity, compatibility, portal schema, privacy, and governance fields')
PYMANIFEST
