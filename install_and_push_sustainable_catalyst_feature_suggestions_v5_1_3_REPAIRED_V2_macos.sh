...
' "$LAYOUT_OUTPUT"
if ! printf '%s
' "$LAYOUT_OUTPUT" | grep -Fq 'v5.2.0 header repair contract passed (6 checks).'; then
  echo "ERROR: The v5.2.0 duplicate-header repair contract did not pass all 6 checks."
  exit 1
fi
