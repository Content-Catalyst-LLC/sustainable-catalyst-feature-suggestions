...
' "$LAYOUT_OUTPUT"
if ! printf '%s
' "$LAYOUT_OUTPUT" | grep -Fq 'v5.2.1 header repair contract passed (6 checks).'; then
  echo "ERROR: The v5.2.1 duplicate-header repair contract did not pass all 6 checks."
  exit 1
fi
