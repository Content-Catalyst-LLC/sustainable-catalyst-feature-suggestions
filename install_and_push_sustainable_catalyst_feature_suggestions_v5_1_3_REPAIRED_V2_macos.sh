...
VERSION="5.2.2"
INSTALLER_REVISION="REPAIRED_V2"
RELEASE_NAME="Compact Support Library, Dedicated Knowledge Base Page, and Shortcode Separation"
...
files=list(root.glob('sustainable-catalyst-feature-suggestions-v5.2.2-repo*.zip'))
files+=list(root.glob('sustainable-catalyst-feature-suggestions-v5.2.2-release-bundle*.zip'))
if files:
...
if [ -z "$SOURCE_ARCHIVE" ] || [ ! -f "$SOURCE_ARCHIVE" ]; then
  echo "ERROR: No v5.2.2 repository ZIP or release bundle was found in ~/Downloads."
  exit 1
fi
...
INNER_ZIP="$(find "$STAGE_DIR" -maxdepth 8 -type f -name 'sustainable-catalyst-feature-suggestions-v5.2.2-repo*.zip' -print -quit)"
if [ -n "$INNER_ZIP" ]; then
...
if [ -z "$MANIFEST_PATH" ]; then
  echo "ERROR: Could not locate the v5.2.2 repository root."
  exit 1
fi
...
echo "Creating safety backup: $BACKUP_PATH"
(
  cd "$CLONE_DIR"
  zip -qr "$BACKUP_PATH" . -x '.git/*' '*/.DS_Store' '*/__MACOSX/*' '*/__pycache__/*' '*/.pytest_cache/*'
)
...
if [ ! -f "$MAIN_PLUGIN" ] || [ ! -f "$KB_CLASS" ] || [ ! -f "$KB_CSS" ] || [ ! -f "$KB_JS" ] || [ ! -f "$KB_CORPUS" ] || [ ! -f "$BACKEND_MAIN" ]; then
  echo "ERROR: Required WordPress, Knowledge Base, corpus, or FastAPI v5.2.2 files are missing."
  exit 1
fi
...
kb=Path(sys.argv[2]).read_text(encoding='utf-8')
backend=Path(sys.argv[3]).read_text(encoding='utf-8')
wp=re.search(r'^[ \t*]*Version:\s*([0-9.]+)', plugin, re.M)
kv=re.search(r"const\s+VERSION\s*=\s*['\"]([^'\"]+)['\"]", kb)
bv=re.search(r"^VERSION\s*=\s*['\"]([^'\"]+)['\"]", backend, re.M)
print(f"{wp.group(1) if wp else ''}|{kv.group(1) if kv else ''}|{bv.group(1) if bv else ''}")
PY
)"
IFS='|' read -r WP_VERSION KB_VERSION BACKEND_VERSION <<EOF
$MARKER_RESULT
EOF

echo "Detected versions: WordPress=${WP_VERSION:-missing}; Knowledge Base=${KB_VERSION:-missing}; FastAPI=${BACKEND_VERSION:-missing}"
if [ "$WP_VERSION" != "5.2.2" ] || [ "$KB_VERSION" != "5.2.2" ]; then
  echo "ERROR: Expected WordPress and Knowledge Base v5.2.2."
  exit 1
fi

# v5.2.2 is a WordPress/Knowledge Base presentation patch. The unchanged
...
if [ ! -f tests/test-v513-header-repair.php ]; then
  echo "ERROR: The v5.2.2 publication-parity Knowledge Base contract test is missing."
  exit 1
fi
LAYOUT_OUTPUT="$(php tests/test-v513-header-repair.php)"
printf '%s
' "$LAYOUT_OUTPUT"
if ! printf '%s
' "$LAYOUT_OUTPUT" | grep -Fq 'v5.2.2 header repair contract passed (6 checks).'; then
  echo "ERROR: The v5.2.2 duplicate-header repair contract did not pass all 6 checks."
  exit 1
fi
...
if ! grep -Fq 'Version: 5.2.2' "$HEADER_FILE"; then
  echo "ERROR: WordPress distribution ZIP does not contain plugin version 5.2.2."
  exit 1
fi
if ! grep -Fq "const VERSION = '5.2.2';" "$KB_ZIP_FILE"; then
  echo "ERROR: WordPress distribution ZIP does not contain the v5.2.2 Knowledge Base class."
  exit 1
fi
...
if git diff --cached --quiet; then
  echo "No repository changes remain. v5.2.2 may already be installed."
else
  git commit -m "Release Feature Suggestions v5.2.2 — Compact Support Library, Dedicated Knowledge Base Page, and Shortcode Separation"
fi
...
echo
echo "Feature Suggestions v5.2.2 was validated, committed, and pushed successfully."
