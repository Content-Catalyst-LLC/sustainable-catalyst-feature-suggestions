...
contract_log="$(mktemp "${TMPDIR:-/tmp}/scpsf-v733-contract.XXXXXX")"
...
php -r '$plugin=file_get_contents($argv[1]);if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform[[:space:]]*$/m",$plugin)){exit(1);}if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Version:[[:space:]]*7\\.3\\.3[[:space:]]*$/m",$plugin)){exit(1);}if(!preg_match("/const VERSION = '\''7\\.3\\.3'\'';/",$plugin)){exit(1);}if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Text Domain:[[:space:]]*sustainable-catalyst-feature-suggestions[[:space:]]*$/m",$plugin)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity or compatibility check failed."
...
versioned=json.loads((r/'feature_suggestions_manifest-v7.3.3.json').read_text())
historical=json.loads((r/'feature_suggestions_manifest-v7.2.1.json').read_text())
release=json.loads((r/'release-manifest-v7.3.3.json').read_text())
example=json.loads((r/'examples/release-board-v7.3.3.json').read_text())
schema=json.loads((r/'schemas/scfs-release-board-v1.schema.json').read_text())
assert current==versioned
assert current['version']=='7.3.3'
assert current['release_name']=='Release Console Reliability and Presentation Repair'
...
assert release['version']=='7.3.3'
assert release['release_name']=='Release Console Reliability and Presentation Repair'
assert release['compatibility']['database_migration_required'] is False
assert example['schema']==board['schema']
assert example['version']=='7.3.3'
assert schema['properties']['version']['const']=='7.3.3'
...
BOARD_CSS="$PLUGIN/assets/release-board-v7.3.3.css"
BOARD_JS="$PLUGIN/assets/release-console-v7.3.3.js"
...
grep -Fq "const VERSION = '7.3.3';" "$BOARD_CLASS" || fail "Release board version mismatch."
...
INSTALLER="$ROOT/install_and_push_sustainable_catalyst_product_support_feedback_v7_3_3_macos.sh"
[ -f "$INSTALLER" ] || fail "Missing v7.3.3 installer."
...
if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$INSTALLER" "$ROOT/validate_v7_3_3.sh"; then fail "Release scripts require Bash 4."; fi
