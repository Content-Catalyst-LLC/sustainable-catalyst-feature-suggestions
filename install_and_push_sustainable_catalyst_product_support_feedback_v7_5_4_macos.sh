...
set -Eeuo pipefail
VERSION="7.5.4"
INSTALLER_REVISION="V7_5_4_GITHUB_CONNECTION_FOOTER_CONTROLS"
RELEASE_TITLE="Administrator GitHub Connection and Console Link Controls"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v7_5_4.sh"
REPO_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.5.4-repository.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v7.5.4-release-bundle.zip"
SUMS_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.5.4-artifacts.sha256"
...
[ -n "$ARCHIVE" ] || fail "Place the v7.5.4 repository ZIP or release bundle beside this installer or in ~/Downloads."
...
if [ -n "$REPO_ZIP" ]; then
  ...
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.5.4-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.5.4-repository' -print -quit)"
fi
[ -n "${SOURCE:-}" ] || fail "Could not locate the v7.5.4 repository source."
...
grep -Fq 'VERSION="7.5.4"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "Wrong validator version."
grep -Fq 'Version: 7.5.4' "$SOURCE/wordpress/$PLUGIN_SLUG/$PLUGIN_SLUG.php" || fail "Wrong plugin version."
...
grep -Fq 'MutationObserver' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-console-v7.5.4.js" || fail "Dynamic Release Console initialization is missing."
grep -Fq '[data-console-active="true"]' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.5.4.css" || fail "Stable Release Console screen layout is missing."
grep -Fq -- '--scfs-release-console-columns:' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.5.4.css" || fail "Shared responsive Release Console grid is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.5.4.js" ] || fail "Plugin Discovery mapping JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.5.4.css" ] || fail "Plugin Discovery mapping stylesheet is missing."
...
grep -Fq 'No GitHub Release is published yet; repository and commit evidence were synchronized.' "$GITHUB_CLASS" || fail "No-release synchronization state is missing."
...
grep -Fq "\$record['version_source'] = 'github_release';" "$GITHUB_CLASS" || fail "GitHub release-to-console version propagation is missing."
...
if [ "${SCPSF_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v7.5.4 Administrator GitHub Connection and Console Link Controls package and preserved the WordPress plugin identity.\n'
  exit 0
fi
...
printf '==> Installing v7.5.4 source with checksum verification\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v7.5.4.\n'
else
  git commit -m "Product Support and Feedback Platform v7.5.4 — $RELEASE_TITLE"
fi
...
push_release_tag
printf '\nSUCCESS: v7.5.4 validated, installed, committed, tagged, and pushed.\n'
