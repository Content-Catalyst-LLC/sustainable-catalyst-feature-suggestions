...
VERSION="5.7.0"
INSTALLER_REVISION="V5_7_0_R1_SUPPORT_ANALYTICS_DOCUMENTATION_EFFECTIVENESS"
RELEASE_NAME="Support Analytics and Documentation Effectiveness"
REPOSITORY="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-feature-suggestions.git"
DOWNLOADS_DIR="${HOME}/Downloads"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/scfs-v570.XXXXXX")"
...
[ -n "$PYTHON_BIN" ] || fail 'Python 3.12 or 3.13 is required for v5.7.0 validation. Install one with: brew install python@3.13'
...
patterns = ('sustainable-catalyst-product-support-feedback-platform-v5.7.0-release-bundle*.zip','sustainable-catalyst-feature-suggestions-v5.7.0-repo*.zip')
files=[]
for pattern in patterns: files.extend(root.glob(pattern))
print(max(files, key=lambda p: p.stat().st_mtime) if files else '')
PYSELECT
)"
[ -f "$SOURCE_ARCHIVE" ] || fail 'No v5.7.0 repository ZIP or release bundle was found in ~/Downloads.'
printf 'Using release archive: %s\n' "$SOURCE_ARCHIVE"
unzip -q "$SOURCE_ARCHIVE" -d "$STAGE_DIR"
INNER_ZIP="$(find "$STAGE_DIR" -maxdepth 8 -type f -name 'sustainable-catalyst-feature-suggestions-v5.7.0-repo*.zip' -print -quit)"
if [ -n "$INNER_ZIP" ]; then
...
analytics=manifest.get('support_analytics',{})
assert analytics.get('schema')=='scfs-support-analytics-documentation-effectiveness/1.0'
assert analytics.get('administrator_only') is True
assert analytics.get('raw_search_text_exposed') is False
assert analytics.get('private_case_content_exposed') is False
assert analytics.get('automatic_publication') is False
assert analytics.get('automatic_issue_resolution') is False
assert analytics.get('automatic_roadmap_changes') is False
assert analytics.get('human_review_required') is True
assert manifest.get('validation',{}).get('fastapi_tests',0)>=111
PYMAN
...
for required in \
  ...
  wordpress/sustainable-catalyst-feature-suggestions/includes/class-scfs-support-analytics-documentation-effectiveness.php \
  ...
  tests/test-v570-bootstrap.php tests/test-v570-engine.php tests/test-v570-admin-api.php \
  tests/test-v570-privacy-governance.php tests/test-v570-compatibility.php tests/test-v570-release-artifacts.php \
  ...
  release-manifest-v5.7.0.json validate_v5_7_0.sh; do
...
PYTHON_BIN="$VENV_DIR/bin/python" ./validate_v5_7_0.sh
...
printf 'Support Analytics: WordPress Admin → Support & Feedback → Support Analytics\n'
