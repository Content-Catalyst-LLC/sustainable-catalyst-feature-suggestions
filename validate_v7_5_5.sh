...
[ "$php_count" = "51" ] || fail "Expected 51 plugin PHP files, got $php_count."
...
contract_log="$(mktemp "${TMPDIR:-/tmp}/scpsf-v755-contract.XXXXXX")"
...
rm -f "$contract_log"
[ "$contract_count" = "277" ] || fail "Expected 277 PHP contracts, got $contract_count."
printf 'PASS - %d PHP contract files executed sequentially\n' "$contract_count"

printf '==> Runtime identity and preserved WordPress slug\n'
php -r '$p=file_get_contents($argv[1]); if(!preg_match("/Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform/",$p)||!preg_match("/Version:[[:space:]]*7\\.5\\.5/",$p)||!preg_match("/const VERSION = '\''7\\.5\\.5'\'';/",$p)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity mismatch."
...
[ "$js_count" = "42" ] || fail "Expected 42 JavaScript files, got $js_count."
...
COUNTS
[ "$json_count" = "227" ] || fail "Expected 227 JSON files, got $json_count."
...
[ "$css_count" = "53" ] || fail "Expected 53 CSS layers, got $css_count."
printf 'PASS - %d balanced CSS layers\n' "$css_count"

printf '==> v7.5.5 release metadata, GitHub settings, and footer contracts\n'
"$PYTHON_BIN" - "$ROOT" <<'PY'
from pathlib import Path
import json,sys
r=Path(sys.argv[1]); version='7.5.5'; name='GitHub Tag Fallback and Unified Console Administration'
current=json.loads((r/'feature_suggestions_manifest.json').read_text())
versioned=json.loads((r/'feature_suggestions_manifest-v7.5.5.json').read_text())
release=json.loads((r/'release-manifest-v7.5.5.json').read_text())
...
assert current['validation']['php_contract_files'] == 277
assert current['validation']['fastapi_tests'] == 318
assert current['validation']['javascript_files'] == 42
assert current['validation']['css_layers'] == 53
assert current['validation']['json_files'] == 227
...
php "$ROOT/tests/test-v753-active-plugin-github-connections.php" >/dev/null || fail "v7.5.5 active-plugin source contract failed."
php "$ROOT/tests/test-v753-github-sync-runtime.php" >/dev/null || fail "v7.5.5 GitHub synchronization runtime contract failed."
php "$ROOT/tests/test-v754-github-settings-and-footer.php" >/dev/null || fail "v7.5.5 settings and footer contract failed."
php "$ROOT/tests/test-v754-credential-runtime.php" >/dev/null || fail "v7.5.5 encrypted credential runtime contract failed."
php "$ROOT/tests/test-v755-github-tag-fallback-runtime.php" >/dev/null || fail "v7.5.5 semantic Git tag fallback runtime contract failed."
php "$ROOT/tests/test-v755-unified-github-console.php" >/dev/null || fail "v7.5.5 unified GitHub Console contract failed."
printf 'PASS - active plugins, encrypted GitHub connection, semantic-tag fallback, schedule repair, console updates, and unified footer controls\n'
...
if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$ROOT/validate_v7_5_5.sh"; then fail "Validator requires Bash 4."; fi
bash -n "$ROOT/validate_v7_5_5.sh" || fail "Validator shell syntax failed."
