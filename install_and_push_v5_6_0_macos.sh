...
VERSION="5.6.0"
INSTALLER_REVISION="V5_6_0_R1_FEEDBACK_INTELLIGENCE_PRODUCT_SIGNALS"
RELEASE_NAME="Feedback Intelligence and Product Signals"
REPOSITORY="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-feature-suggestions.git"
DOWNLOADS_DIR="${HOME}/Downloads"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/scfs-v560.XXXXXX")"
...
[ -n "$PYTHON_BIN" ] || fail 'Python 3.12 or 3.13 is required for v5.6.0 validation. Install one with: brew install python@3.13'
...
patterns = (
    'sustainable-catalyst-product-support-feedback-platform-v5.6.0-release-bundle*.zip',
    'sustainable-catalyst-feature-suggestions-v5.6.0-repo*.zip',
)
files = []
...
[ -f "$SOURCE_ARCHIVE" ] || fail 'No v5.6.0 repository ZIP or release bundle was found in ~/Downloads.'
printf 'Using release archive: %s\n' "$SOURCE_ARCHIVE"
unzip -q "$SOURCE_ARCHIVE" -d "$STAGE_DIR"

INNER_ZIP="$(find "$STAGE_DIR" -maxdepth 8 -type f -name 'sustainable-catalyst-feature-suggestions-v5.6.0-repo*.zip' -print -quit)"
if [ -n "$INNER_ZIP" ]; then
...
signals = manifest.get('feedback_product_signals', {})
assert signals.get('schema') == 'scfs-feedback-product-signals/1.0'
assert signals.get('administrator_only') is True
assert signals.get('raw_search_text_exposed') is False
assert signals.get('private_case_content_exposed') is False
assert signals.get('automatic_roadmap_changes') is False
assert signals.get('automatic_issue_declaration') is False
assert signals.get('automatic_publication') is False
assert signals.get('human_review_required') is True
validation = manifest.get('validation', {})
assert validation.get('status') == 'passed'
assert validation.get('fastapi_tests', 0) >= 104
PY
...
for required in \
  ...
  wordpress/sustainable-catalyst-feature-suggestions/includes/class-scfs-feedback-product-signals.php \
  ...
  tests/test-v560-bootstrap.php \
  tests/test-v560-engine.php \
  tests/test-v560-admin-api.php \
  tests/test-v560-privacy-governance.php \
  tests/test-v560-backend.php \
  tests/test-v560-compatibility.php \
  tests/test-v560-release-artifacts.php \
...
PYTHON_BIN="$VENV_DIR/bin/python" ./validate_v5_6_0.sh
...
printf 'Product Signals: WordPress Admin → Support & Feedback → Product Signals\n'
