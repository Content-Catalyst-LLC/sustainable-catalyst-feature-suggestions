...
contract_log="$(mktemp "${TMPDIR:-/tmp}/scpsf-v754-contract.XXXXXX")"
...
rm -f "$contract_log"
[ "$contract_count" = "275" ] || fail "Expected 275 PHP contracts, got $contract_count."
printf 'PASS - %d PHP contract files executed sequentially\n' "$contract_count"

printf '==> Runtime identity and preserved WordPress slug\n'
php -r '$p=file_get_contents($argv[1]); if(!preg_match("/Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform/",$p)||!preg_match("/Version:[[:space:]]*7\\.5\\.4/",$p)||!preg_match("/const VERSION = '\''7\\.5\\.4'\'';/",$p)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity mismatch."
...
[ "$js_count" = "40" ] || fail "Expected 40 JavaScript files, got $js_count."
...
COUNTS
[ "$json_count" = "219" ] || fail "Expected 219 JSON files, got $json_count."
...
[ "$css_count" = "51" ] || fail "Expected 51 CSS layers, got $css_count."
printf 'PASS - %d balanced CSS layers\n' "$css_count"

printf '==> v7.5.4 release metadata, GitHub settings, and footer contracts\n'
"$PYTHON_BIN" - "$ROOT" <<'PY'
from pathlib import Path
import json,sys
r=Path(sys.argv[1]); version='7.5.4'; name='Administrator GitHub Connection and Console Link Controls'
current=json.loads((r/'feature_suggestions_manifest.json').read_text())
versioned=json.loads((r/'feature_suggestions_manifest-v7.5.4.json').read_text())
release=json.loads((r/'release-manifest-v7.5.4.json').read_text())
...
assert current['validation']['php_contract_files'] == 275
assert current['validation']['fastapi_tests'] == 318
assert current['validation']['javascript_files'] == 40
assert current['validation']['css_layers'] == 51
assert current['validation']['json_files'] == 219
...
for key in ('encrypted_wordpress_token','wp_config_override_preserved','environment_override_preserved','token_never_rendered','mapped_repository_test','sync_all_action'):
...
php "$ROOT/tests/test-v753-active-plugin-github-connections.php" >/dev/null || fail "v7.5.4 active-plugin source contract failed."
php "$ROOT/tests/test-v753-github-sync-runtime.php" >/dev/null || fail "v7.5.4 GitHub synchronization runtime contract failed."
php "$ROOT/tests/test-v754-github-settings-and-footer.php" >/dev/null || fail "v7.5.4 settings and footer contract failed."
php "$ROOT/tests/test-v754-credential-runtime.php" >/dev/null || fail "v7.5.4 encrypted credential runtime contract failed."
printf 'PASS - active plugins, encrypted GitHub connection, exact diagnostics, console updates, and editable footer links\n'
...
if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$ROOT/validate_v7_5_4.sh"; then fail "Validator requires Bash 4."; fi
bash -n "$ROOT/validate_v7_5_4.sh" || fail "Validator shell syntax failed."
