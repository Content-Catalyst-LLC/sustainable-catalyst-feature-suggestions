...
set -Eeuo pipefail
VERSION="7.6.1"
INSTALLER_REVISION="V7_6_1_RELEASE_OPERATIONS_STABILIZATION"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scpsf-v761.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
[ -f "$SOURCE/tests/test-v761-github-diagnostics-runtime.php" ] || fail "v7.6.1 GitHub recovery runtime contract is missing."
[ -f "$SOURCE/tests/test-v761-release-operations-stabilization.php" ] || fail "v7.6.1 stabilization contract is missing."
