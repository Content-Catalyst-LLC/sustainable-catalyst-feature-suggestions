...
if [ -z "$MANIFEST_PATH" ]; then
  ...
  if [ -z "$INNER_ZIP" ]; then
    echo "ERROR: The release archive does not contain the v5.2.1 repository."
    exit 1
  fi
...
if [ ! -f tests/test-v512-article-header.php ]; then
  echo "ERROR: The v5.2.1 publication-parity Knowledge Base contract test is missing."
  exit 1
fi
...
if ! printf '%s\n' "$LAYOUT_OUTPUT" | grep -Fq 'PASS - 11 Knowledge Base article header checks'; then
  echo "ERROR: The v5.2.1 publication-parity and expanded-default contract did not pass all 11 checks."
  exit 1
fi
...
if ! grep -Fq "const VERSION = '5.2.1';" "$KB_ZIP_FILE"; then
  echo "ERROR: WordPress distribution ZIP does not contain the v5.2.1 full-width Knowledge Base class."
  exit 1
fi
...
if git diff --cached --quiet; then
  ...
  git commit -m "Release Feature Suggestions v5.2.1 — Knowledge Base Publication-Parity Article Experience"
fi
