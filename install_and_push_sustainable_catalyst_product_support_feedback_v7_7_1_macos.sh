...
set -Eeuo pipefail
VERSION="7.7.1"
INSTALLER_REVISION="V7_7_1_PLUGIN_DISCOVERY_INTELLIGENCE"
RELEASE_TITLE="Plugin Discovery Intelligence"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v7_7_1.sh"
REPO_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.7.1-repository.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v7.7.1-release-bundle.zip"
SUMS_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.7.1-artifacts.sha256"
...
[ -n "$ARCHIVE" ] || fail "Place the v7.7.1 repository ZIP or release bundle beside this installer or in ~/Downloads."
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scpsf-v771.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
if [ -n "$REPO_ZIP" ]; then
  ...
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.7.1-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.7.1-repository' -print -quit)"
fi
[ -n "${SOURCE:-}" ] || fail "Could not locate the v7.7.1 repository source."
...
grep -Fq 'VERSION="7.7.1"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "Wrong validator version."
grep -Fq 'Version: 7.7.1' "$SOURCE/wordpress/$PLUGIN_SLUG/$PLUGIN_SLUG.php" || fail "Wrong plugin version."
...
grep -Fq 'MutationObserver' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-console-v7.7.1.js" || fail "Dynamic Release Console initialization is missing."
grep -Fq '[data-console-active="true"]' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.7.1.css" || fail "Stable Release Console screen layout is missing."
grep -Fq -- '--scfs-release-console-columns:' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.7.1.css" || fail "Shared responsive Release Console grid is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.7.1.js" ] || fail "Plugin Discovery mapping JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.7.1.css" ] || fail "Plugin Discovery mapping stylesheet is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-operations-v7.7.1.js" ] || fail "Release Operations JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-operations-v7.7.1.css" ] || fail "Release Operations stylesheet is missing."
...
[ -f "$REGISTRY_ADMIN_CLASS" ] || fail "Plugin Discovery Intelligence controller is missing."
grep -Fq "const VERSION = '7.7.1';" "$REGISTRY_ADMIN_CLASS" || fail "Plugin Discovery Intelligence version is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/canonical-product-registry-admin-v7.7.1.js" ] || fail "Registry administration JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/canonical-product-registry-admin-v7.7.1.css" ] || fail "Registry administration stylesheet is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.7.1.js" ] || fail "Plugin Discovery Intelligence JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.7.1.css" ] || fail "Plugin Discovery Intelligence stylesheet is missing."
[ -f "$SOURCE/schemas/scfs-plugin-discovery-intelligence-v1.schema.json" ] || fail "Plugin Discovery Intelligence schema is missing."
[ -f "$SOURCE/tests/test-v771-plugin-discovery-intelligence.php" ] || fail "Plugin Discovery Intelligence source contract is missing."
[ -f "$SOURCE/tests/test-v771-plugin-discovery-runtime.php" ] || fail "Plugin Discovery Intelligence runtime contract is missing."
...
if [ "${SCPSF_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v7.7.1 Plugin Discovery Intelligence package and preserved the WordPress plugin identity.\n'
  exit 0
fi
...
printf '==> Installing v7.7.1 source with checksum verification\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v7.7.1.\n'
else
  git commit -m "Product Support and Feedback Platform v7.7.1 — $RELEASE_TITLE"
fi
...
push_release_tag
printf '\nSUCCESS: v7.7.1 validated, installed, committed, tagged, and pushed.\n'
