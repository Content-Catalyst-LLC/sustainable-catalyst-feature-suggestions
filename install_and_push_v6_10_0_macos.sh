...
set -Eeuo pipefail
VERSION="6.10.0"
INSTALLER_REVISION="V6_10_0_CHECKSUM_SYNC_PARITY"
RELEASE_TITLE="Institutional Workspaces and Access Governance"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v6_10_0.sh"
REPO_ARCHIVE="sustainable-catalyst-feature-suggestions-v6.10.0-repo.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v6.10.0-release-bundle.zip"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
[ -n "$ARCHIVE" ]||fail "Place the v6.10.0 repository ZIP or release bundle beside this installer or in ~/Downloads."
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scfs-v6100.XXXXXX")";trap 'rm -rf "$TMP"' EXIT
...
if [ -n "$REPO_ZIP" ];then mkdir -p "$TMP/repository";unzip -q "$REPO_ZIP" -d "$TMP/repository";SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-feature-suggestions-v6.10.0-repository' -print -quit)";else SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-feature-suggestions-v6.10.0-repository' -print -quit)";fi
[ -n "${SOURCE:-}" ]||fail "Could not locate the v6.10.0 repository source."
[ -f "$SOURCE/$VALIDATOR_SCRIPT" ]||fail "Missing $VALIDATOR_SCRIPT.";[ -f "$SOURCE/backend/requirements-validation.txt" ]||fail "Missing validation dependencies.";[ -f "$SOURCE/wordpress/sustainable-catalyst-feature-suggestions/includes/class-scfs-help-desk-institutional-workspaces.php" ]||fail "Missing institutional workspace implementation.";[ -f "$SOURCE/schemas/scfs-help-desk-institutional-workspaces-v1.schema.json" ]||fail "Missing institutional workspace schema."
grep -Fq 'VERSION="6.10.0"' "$SOURCE/$VALIDATOR_SCRIPT"||fail "Wrong validator version.";grep -Fq 'CONTRACT_EXECUTION_MODE="sequential-state-safe"' "$SOURCE/$VALIDATOR_SCRIPT"||fail "Missing state-safe contract mode.";grep -Fq 'Version: 6.10.0' "$SOURCE/wordpress/sustainable-catalyst-feature-suggestions/sustainable-catalyst-feature-suggestions.php"||fail "Wrong plugin version."
...
if [ "${SCFS_PREFLIGHT_ONLY:-0}" = "1" ];then printf 'PREFLIGHT PASSED: selected the v6.10.0 archive and checksum-sync parity installer.\n';exit 0;fi
...
printf '==> Pushing tag %s\n' "$TAG";push_release_tag;printf '\nSUCCESS: v6.10.0 synchronized by checksum, parity-verified, validated, committed, tagged, and pushed.\nLocal repository: %s\n' "$REPO_DIR"
