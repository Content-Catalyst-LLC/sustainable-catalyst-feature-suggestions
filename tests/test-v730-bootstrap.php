...
$checks = array(
    ...
    'runtime version' => strpos($main, "const VERSION = '7.3.0';") !== false,
    ...
    'board version' => strpos($board, "const VERSION = '7.3.0';") !== false,
    'board schema' => strpos($board, 'scfs-release-board/1.0') !== false,
    'shortcode' => strpos($board, "const SHORTCODE = 'sc_release_board';") !== false,
    'registry version' => strpos($registry, "const VERSION = '7.3.0';") !== false,
    'legacy WordPress identity' => strpos($main, 'Text Domain: sustainable-catalyst-feature-suggestions') !== false,
);
foreach ($checks as $label => $ok) {
    if (!$ok) { fwrite(STDERR, "FAIL: {$label}\n"); exit(1); }
}
echo "v7.3.0 release board bootstrap contract passed.\n";
