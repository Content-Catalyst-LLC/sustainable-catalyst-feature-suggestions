...
if ($current !== $versioned || ($current['version'] ?? '') !== '7.6.2' || ($current['release_name'] ?? '') !== 'Product Connection Editor') {
    fwrite(STDERR, "FAIL current/versioned manifest parity\n"); exit(1);
}
if (($release['version'] ?? '') !== '7.6.2' || ($release['release_name'] ?? '') !== 'Product Connection Editor') {
