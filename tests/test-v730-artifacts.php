...
if ($current !== $versioned || ($current['version'] ?? '') !== '7.5.4' || ($current['release_name'] ?? '') !== 'Administrator GitHub Connection and Console Link Controls') {
    fwrite(STDERR, "FAIL current/versioned manifest parity\n"); exit(1);
}
if (($release['version'] ?? '') !== '7.5.4' || ($release['release_name'] ?? '') !== 'Administrator GitHub Connection and Console Link Controls') {
