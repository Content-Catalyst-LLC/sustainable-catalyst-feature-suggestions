...
if ($current !== $versioned || ($current['version'] ?? '') !== '7.6.1' || ($current['release_name'] ?? '') !== 'Release Operations Stabilization') {
    fwrite(STDERR, "FAIL current/versioned manifest parity\n"); exit(1);
}
if (($release['version'] ?? '') !== '7.6.1' || ($release['release_name'] ?? '') !== 'Release Operations Stabilization') {
