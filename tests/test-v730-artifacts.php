...
if ($current !== $versioned || ($current['version'] ?? '') !== '7.6.0' || ($current['release_name'] ?? '') !== 'Release Operations Administration and Sync Governance') {
    fwrite(STDERR, "FAIL current/versioned manifest parity\n"); exit(1);
}
if (($release['version'] ?? '') !== '7.6.0' || ($release['release_name'] ?? '') !== 'Release Operations Administration and Sync Governance') {
