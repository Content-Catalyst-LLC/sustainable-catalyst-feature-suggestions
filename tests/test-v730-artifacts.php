...
if ($current !== $versioned || ($current['version'] ?? '') !== '7.7.1' || ($current['release_name'] ?? '') !== 'Plugin Discovery Intelligence') {
    fwrite(STDERR, "FAIL current/versioned manifest parity\n"); exit(1);
}
if (($release['version'] ?? '') !== '7.7.1' || ($release['release_name'] ?? '') !== 'Plugin Discovery Intelligence') {
