...
if ($current !== $versioned || ($current['version'] ?? '') !== '7.8.0' || ($current['release_name'] ?? '') !== 'GitHub Release Intelligence') {
    fwrite(STDERR, "FAIL current/versioned manifest parity\n"); exit(1);
}
if (($release['version'] ?? '') !== '7.8.0' || ($release['release_name'] ?? '') !== 'GitHub Release Intelligence') {
