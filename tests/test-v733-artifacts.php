...
$files = array(
    ...
    'docs/release-console-v7.5.5.md',
    'docs/release-board-shortcode-v7.5.5.md',
    'docs/release-telemetry-v7.5.5.md',
    'examples/release-board-v7.5.5.json',
    'examples/release-console-v7.5.5.json',
    'examples/release-telemetry-v7.5.5.json',
...
if (($current['version'] ?? '') !== '7.5.5' || ($current['release_name'] ?? '') !== 'GitHub Tag Fallback and Unified Console Administration') { fwrite(STDERR, "FAIL manifest identity\n"); exit(1); }
...
if (($release['version'] ?? '') !== '7.5.5' || empty($release['validation']['stable_screen_height']) || empty($release['validation']['multiple_instances_supported'])) { fwrite(STDERR, "FAIL release manifest\n"); exit(1); }
if (($schema['properties']['version']['const'] ?? '') !== '7.5.5') { fwrite(STDERR, "FAIL schema version\n"); exit(1); }
echo "v7.5.5 release artifacts contract passed.\n";
