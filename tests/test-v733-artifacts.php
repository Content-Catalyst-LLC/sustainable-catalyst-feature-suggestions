...
$files = array(
    'RELEASE_NOTES_7.3.3.md',
    'SUSTAINABLE_CATALYST_PRODUCT_SUPPORT_AND_FEEDBACK_PLATFORM_V7.3.3_RELEASE_NOTES.md',
    'docs/release-console-v7.3.3.md',
    'docs/release-board-shortcode-v7.3.3.md',
    'docs/release-telemetry-v7.3.3.md',
    'examples/release-board-v7.3.3.json',
    'examples/release-console-v7.3.3.json',
    'examples/release-telemetry-v7.3.3.json',
    'feature_suggestions_manifest-v7.3.3.json',
    'release-manifest-v7.3.3.json',
...
$versioned = json_decode(file_get_contents($root . '/feature_suggestions_manifest-v7.3.3.json'), true);
$release = json_decode(file_get_contents($root . '/release-manifest-v7.3.3.json'), true);
...
if (($current['version'] ?? '') !== '7.3.3' || ($current['release_name'] ?? '') !== 'Release Console Reliability and Presentation Repair') { fwrite(STDERR, "FAIL manifest identity\n"); exit(1); }
...
if (($release['version'] ?? '') !== '7.3.3' || empty($release['validation']['stable_screen_height']) || empty($release['validation']['multiple_instances_supported'])) { fwrite(STDERR, "FAIL release manifest\n"); exit(1); }
if (($schema['properties']['version']['const'] ?? '') !== '7.3.3') { fwrite(STDERR, "FAIL schema version\n"); exit(1); }
echo "v7.3.3 release artifacts contract passed.\n";
