...
$files = array(
    ...
    'docs/release-console-v7.6.0.md',
    'docs/release-board-shortcode-v7.6.0.md',
    'docs/release-telemetry-v7.6.0.md',
    'examples/release-board-v7.6.0.json',
    'examples/release-console-v7.6.0.json',
    'examples/release-telemetry-v7.6.0.json',
...
if (($current['version'] ?? '') !== '7.6.0' || ($current['release_name'] ?? '') !== 'Release Operations Administration and Sync Governance') { fwrite(STDERR, "FAIL manifest identity\n"); exit(1); }
...
if (($release['version'] ?? '') !== '7.6.0' || empty($release['validation']['stable_screen_height']) || empty($release['validation']['multiple_instances_supported'])) { fwrite(STDERR, "FAIL release manifest\n"); exit(1); }
if (($schema['properties']['version']['const'] ?? '') !== '7.6.0') { fwrite(STDERR, "FAIL schema version\n"); exit(1); }
echo "v7.6.0 release artifacts contract passed.\n";
