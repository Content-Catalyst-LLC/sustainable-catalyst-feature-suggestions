...
$files = array(
    'RELEASE_NOTES_7.4.0.md',
    'SUSTAINABLE_CATALYST_PRODUCT_SUPPORT_AND_FEEDBACK_PLATFORM_V7.4.0_RELEASE_NOTES.md',
    'docs/release-console-v7.4.0.md',
    'docs/release-board-shortcode-v7.4.0.md',
    'docs/release-telemetry-v7.4.0.md',
    'examples/release-board-v7.4.0.json',
    'examples/release-console-v7.4.0.json',
    'examples/release-telemetry-v7.4.0.json',
    'feature_suggestions_manifest-v7.4.0.json',
    'release-manifest-v7.4.0.json',
...
$versioned = json_decode(file_get_contents($root . '/feature_suggestions_manifest-v7.4.0.json'), true);
$release = json_decode(file_get_contents($root . '/release-manifest-v7.4.0.json'), true);
...
if (($current['version'] ?? '') !== '7.4.0' || ($current['release_name'] ?? '') !== 'Product Registry Governance') { fwrite(STDERR, "FAIL manifest identity\n"); exit(1); }
...
if (($release['version'] ?? '') !== '7.4.0' || empty($release['validation']['stable_screen_height']) || empty($release['validation']['multiple_instances_supported'])) { fwrite(STDERR, "FAIL release manifest\n"); exit(1); }
if (($schema['properties']['version']['const'] ?? '') !== '7.4.0') { fwrite(STDERR, "FAIL schema version\n"); exit(1); }
echo "v7.4.0 release artifacts contract passed.\n";
