...
if (($current['version'] ?? '') !== '7.7.0' || ($current['release_name'] ?? '') !== 'Canonical Product Registry Administration') { fwrite(STDERR, "FAIL manifest identity\n"); exit(1); }
...
if (($release['version'] ?? '') !== '7.7.0' || empty($release['validation']['stable_screen_height']) || empty($release['validation']['multiple_instances_supported'])) { fwrite(STDERR, "FAIL release manifest\n"); exit(1); }
if (($schema['properties']['version']['const'] ?? '') !== '7.7.0') { fwrite(STDERR, "FAIL schema version\n"); exit(1); }
