...
$checks = array(
    ...
    'runtime version constant' => strpos($main, "const VERSION = '5.2.0';") !== false,
    'knowledge base version' => strpos($class, "const VERSION = '5.2.0';") !== false,
    ...
    'product details expanded' => strpos($class, 'data-scfs-kb-product open') !== false,
    'section details expanded' => strpos($class, 'class="scfs-kb-section-folder" open') !== false,
    ...
    'release notes' => file_exists($root . '/RELEASE_NOTES_5.2.0.md'),
);
$failed = array_keys(array_filter($checks, function ($value) { return !$value; }));
foreach ($checks as $label => $ok) echo ($ok ? 'PASS' : 'FAIL') . " - {$label}\n";
if ($failed) { echo 'Failed checks: ' . implode(', ', $failed) . "\n"; exit(1); }
echo 'v5.2.0 support documentation library contract passed (' . count($checks) . " checks).\n";
