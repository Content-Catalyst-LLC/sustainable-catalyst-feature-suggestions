...
$release = json_decode(file_get_contents($root . '/release-manifest-v5.3.0.json'), true);
$checks = array(
    ...
    'manifest current version' => ($manifest['version'] ?? '') === '5.3.0',
    'manifest release name' => ($manifest['release_name'] ?? '') === 'Unified Search and Guided Resolution',
    'release manifest current version' => ($release['version'] ?? '') === '5.3.0',
    'release notes' => file_exists($root . '/RELEASE_NOTES_5.3.0.md'),
...
echo 'v5.3.0 interface and release artifact contract passed (' . count($checks) . " checks).\n";
