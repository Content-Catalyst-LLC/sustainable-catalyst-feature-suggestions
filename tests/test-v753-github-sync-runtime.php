...
$GLOBALS['scfs_http'][$base . '/releases?per_page=1'] = array('response' => array('code' => 200), 'body' => json_encode(array(array('tag_name' => 'v7.6.1', 'name' => 'Release Operations Stabilization', 'html_url' => 'https://github.com/Content-Catalyst-LLC/sustainable-catalyst-product-support-feedback/releases/tag/v7.6.1', 'published_at' => '2026-07-22T22:05:00Z', 'draft' => false, 'prerelease' => false))));
...
$checks = array(
    'public version' => ($result['public_version'] ?? '') === '7.6.1',
    ...
    'release URL' => ($result['github_latest_release_url'] ?? '') === 'https://github.com/Content-Catalyst-LLC/sustainable-catalyst-product-support-feedback/releases/tag/v7.6.1',
    ...
    'release label' => ($result['change_summary'] ?? '') === 'Release Operations Stabilization',
);
foreach ($checks as $label => $passed) {
    if (!$passed) { fwrite(STDERR, "FAIL {$label}\n"); exit(1); }
}
echo "v7.6.1 GitHub-to-console runtime synchronization passed (" . count($checks) . " checks).\n";
