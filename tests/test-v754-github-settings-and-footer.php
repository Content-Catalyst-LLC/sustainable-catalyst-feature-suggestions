...
$manifest = json_decode(file_get_contents($root . '/feature_suggestions_manifest-v7.6.0.json'), true);
$release = json_decode(file_get_contents($root . '/release-manifest-v7.6.0.json'), true);

$checks = array(
    'runtime identity' => strpos($main, 'Version: 7.6.0') !== false && strpos($main, "const VERSION = '7.6.0';") !== false,
    ...
    'manifest identity' => ($manifest['version'] ?? '') === '7.6.0' && ($release['version'] ?? '') === '7.6.0',
...
echo 'v7.6.0 GitHub settings and footer controls contract passed (' . count($checks) . " checks).\n";
