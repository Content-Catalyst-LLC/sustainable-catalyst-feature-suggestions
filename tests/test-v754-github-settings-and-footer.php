...
$manifest = json_decode(file_get_contents($root . '/feature_suggestions_manifest-v7.5.4.json'), true);
$release = json_decode(file_get_contents($root . '/release-manifest-v7.5.4.json'), true);

$checks = array(
    'runtime identity' => strpos($main, 'Version: 7.5.4') !== false && strpos($main, "const VERSION = '7.5.4';") !== false,
    ...
    'exact HTTP error' => strpos($sync, 'GitHub returned HTTP %1$d: %2$s') !== false,
    'no release success' => strpos($sync, 'No GitHub Release is published yet; repository and commit evidence were synchronized.') !== false,
    ...
    'manifest identity' => ($manifest['version'] ?? '') === '7.5.4' && ($release['version'] ?? '') === '7.5.4',
...
echo 'v7.5.4 GitHub settings and footer controls contract passed (' . count($checks) . " checks).\n";
