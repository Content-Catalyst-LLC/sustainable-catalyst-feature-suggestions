...
$manifest = json_decode(file_get_contents($root . '/feature_suggestions_manifest-v7.5.5.json'), true);
$release = json_decode(file_get_contents($root . '/release-manifest-v7.5.5.json'), true);

$checks = array(
    'runtime identity' => strpos($main, 'Version: 7.5.5') !== false && strpos($main, "const VERSION = '7.5.5';") !== false,
    ...
    'manifest identity' => ($manifest['version'] ?? '') === '7.5.5' && ($release['version'] ?? '') === '7.5.5',
...
echo 'v7.5.5 GitHub settings and footer controls contract passed (' . count($checks) . " checks).\n";
