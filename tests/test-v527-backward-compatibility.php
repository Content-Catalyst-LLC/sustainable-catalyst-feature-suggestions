...
$checks = array(
    ...
    'legacy repository slug' => ($manifest['slug'] ?? '') === 'sustainable-catalyst-feature-suggestions',
    ...
    'legacy support shortcode' => strpos($support, "const LEGACY_SHORTCODE = 'scfs_support_center';") !== false,
    'knowledge base shortcode' => strpos($kb, "const SHORTCODE = 'scfs_support_knowledge_base';") !== false,
    'article URLs preserved' => strpos($kb, "'rewrite' => array('slug' => 'support/guides'") !== false,
    'existing data preserved' => ($manifest['compatibility']['database_migration_required'] ?? null) === false,
);
$failed = array_keys(array_filter($checks, static function ($ok) { return !$ok; }));
foreach ($checks as $label => $ok) echo ($ok ? 'PASS' : 'FAIL') . " - {$label}\n";
if ($failed) { fwrite(STDERR, 'Failed checks: ' . implode(', ', $failed) . "\n"); exit(1); }
echo 'v5.2.7 backward compatibility contract passed (' . count($checks) . " checks).\n";
