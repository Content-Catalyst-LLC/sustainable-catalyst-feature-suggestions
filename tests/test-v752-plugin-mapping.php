...
$js = file_get_contents($plugin . '/assets/plugin-discovery-v7.6.2.js');
...
$checks = array(
    'discovery version' => strpos($discovery, "const VERSION = '7.6.2';") !== false,
    ...
    'manifest identity' => ($manifest['version'] ?? '') === '7.6.2' && ($manifest['release_name'] ?? '') === 'Product Connection Editor',
...
echo "v7.6.2 canonical plugin mapping workflow contract passed (" . count($checks) . " checks).\n";
