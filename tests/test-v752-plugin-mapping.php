...
$js = file_get_contents($plugin . '/assets/plugin-discovery-v7.5.5.js');
$css = file_get_contents($plugin . '/assets/plugin-discovery-v7.5.5.css');
$manifest = json_decode(file_get_contents($root . '/feature_suggestions_manifest.json'), true);
$release = json_decode(file_get_contents($root . '/release-manifest-v7.5.5.json'), true);
$checks = array(
    'discovery version' => strpos($discovery, "const VERSION = '7.5.5';") !== false,
    ...
    'manifest identity' => ($manifest['version'] ?? '') === '7.5.5' && ($manifest['release_name'] ?? '') === 'GitHub Tag Fallback and Unified Console Administration',
...
echo "v7.5.5 canonical plugin mapping workflow contract passed (" . count($checks) . " checks).\n";
