...
$js = file_get_contents($plugin . '/assets/plugin-discovery-v7.6.0.js');
$css = file_get_contents($plugin . '/assets/plugin-discovery-v7.6.0.css');
$manifest = json_decode(file_get_contents($root . '/feature_suggestions_manifest.json'), true);
$release = json_decode(file_get_contents($root . '/release-manifest-v7.6.0.json'), true);
$checks = array(
    'discovery version' => strpos($discovery, "const VERSION = '7.6.0';") !== false,
    ...
    'manifest identity' => ($manifest['version'] ?? '') === '7.6.0' && ($manifest['release_name'] ?? '') === 'Release Operations Administration and Sync Governance',
...
echo "v7.6.0 canonical plugin mapping workflow contract passed (" . count($checks) . " checks).\n";
