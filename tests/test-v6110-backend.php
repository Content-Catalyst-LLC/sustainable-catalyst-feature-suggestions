...
");exit(1);}}foreach(array('VERSION = "6.11.0"','evaluate_api_scope','evaluate_webhook_subscription','sign_webhook_delivery','evaluate_delivery_retry','evaluate_external_link','evaluate_integration_privacy','verify_integration_report') as $needle){if(strpos($module,$needle)===false){fwrite(STDERR,"FAIL backend: $needle
");exit(1);}}if(strpos($test,'test_capabilities_endpoint')===false){fwrite(STDERR,"FAIL backend test
");exit(1);}echo "v6.11.0 integration backend contract passed.
";
