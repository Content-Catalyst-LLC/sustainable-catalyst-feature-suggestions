...
$checks = array(
    'plugin version' => strpos($main, 'Version: 7.2.1') !== false,
    ...
    'class version' => strpos($class, "const VERSION = '7.2.1';") !== false,
    'schema' => strpos($class, 'scfs-canonical-product-registry/1.0') !== false,
);
foreach ($checks as $label => $ok) {
    if (!$ok) { fwrite(STDERR, "FAIL: {$label}\n"); exit(1); }
}
echo "v7.2.1 canonical product registry bootstrap contract passed.\n";
