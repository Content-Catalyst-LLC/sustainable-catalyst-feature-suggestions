...
set -Eeuo pipefail
VERSION="7.3.1"
INSTALLER_REVISION="V7_3_1_RELEASE_TELEMETRY"
RELEASE_TITLE="Release Telemetry and Homepage Presentation"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v7_3_1.sh"
REPO_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.3.1-repository.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v7.3.1-release-bundle.zip"
SUMS_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.3.1-artifacts.sha256"
...
[ -n "$ARCHIVE" ] || fail "Place the v7.3.1 repository ZIP or release bundle beside this installer or in ~/Downloads."
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scpsf-v731.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
if [ -n "$REPO_ZIP" ]; then
  ...
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.3.1-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.3.1-repository' -print -quit)"
fi
[ -n "${SOURCE:-}" ] || fail "Could not locate the v7.3.1 repository source."
...
grep -Fq 'VERSION="7.3.1"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "Wrong validator version."
grep -Fq 'Version: 7.3.1' "$SOURCE/wordpress/$PLUGIN_SLUG/$PLUGIN_SLUG.php" || fail "Wrong plugin version."
...
grep -Fq '"public_title": "Release Telemetry"' "$SOURCE/feature_suggestions_manifest.json" || fail "Release Telemetry metadata is missing."
grep -Fq "'layout' => 'terminal'" "$SOURCE/wordpress/$PLUGIN_SLUG/includes/class-scfs-release-board.php" || fail "Terminal Release Telemetry layout is missing."
...
if [ "${SCPSF_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v7.3.1 Release Telemetry and Homepage Presentation package and preserved the WordPress plugin identity.\n'
  exit 0
fi
...
printf '==> Installing v7.3.1 source with checksum verification\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v7.3.1.\n'
else
  git commit -m "Product Support and Feedback Platform v7.3.1 — $RELEASE_TITLE"
fi
...
push_release_tag
printf '\nSUCCESS: v7.3.1 validated, installed, committed, tagged, and pushed.\n'
