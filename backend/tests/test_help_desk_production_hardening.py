...
def test_backup_snapshot_requires_encryption_offsite_and_restore_test():
    good_hash = "a" * 64
    result = evaluate_backup_snapshot(BackupSnapshotEvidence(snapshot_id="snapshot-20260720", created_at="2026-07-20T12:00:00Z", source_version="7.0.0", database_sha256=good_hash, files_sha256=good_hash, encrypted=True, offsite_copy=True, restore_tested=True, age_hours=6))
...
def test_report_integrity():
    payload = {"version": "7.0.0", "security": {"state": "ready"}, "recovery": {"state": "verified"}}
