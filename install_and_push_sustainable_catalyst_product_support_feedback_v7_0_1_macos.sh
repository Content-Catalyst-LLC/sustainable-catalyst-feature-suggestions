...
set -Eeuo pipefail
VERSION="7.0.1"
INSTALLER_REVISION="V7_0_1_REPOSITORY_IDENTITY_MIGRATION"
RELEASE_TITLE="Repository Identity Migration"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v7_0_1.sh"
REPO_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.0.1-repository.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v7.0.1-release-bundle.zip"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scpsf-v701.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
if [ -z "$SUMS_FILE" ]; then
  for candidate in "$SCRIPT_DIR/SHA256SUMS" "$DOWNLOADS/SHA256SUMS"; do
...
if [ -n "$REPO_ZIP" ]; then
  ...
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.0.1-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.0.1-repository' -print -quit)"
fi
[ -n "${SOURCE:-}" ] || fail "Could not locate the v7.0.1 repository source."
...
grep -Fq 'VERSION="7.0.1"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "Wrong validator version."
grep -Fq 'Version: 7.0.1' "$SOURCE/wordpress/$PLUGIN_SLUG/$PLUGIN_SLUG.php" || fail "Wrong plugin version."
...
if [ "${SCPSF_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v7.0.1 migration release and preserved the WordPress plugin identity.\n'
  exit 0
fi
printf '==> Verifying canonical GitHub repository access\n'
if ! git ls-remote "$REMOTE" HEAD >/dev/null 2>&1; then
  fail "The canonical GitHub repository is not reachable. Rename the GitHub repository to sustainable-catalyst-product-support-feedback, confirm SSH access, and run this installer again."
fi
...
printf '==> Installing v7.0.1 source with checksum verification\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v7.0.1.\n'
else
  git commit -m "Product Support and Feedback Platform v7.0.1 — $RELEASE_TITLE"
fi
...
push_release_tag
printf '\nSUCCESS: v7.0.1 validated, migrated to the canonical repository identity, committed, tagged, and pushed.\n'
