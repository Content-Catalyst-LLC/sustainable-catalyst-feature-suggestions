...
contract_log="$(mktemp "${TMPDIR:-/tmp}/scpsf-v771-contract.XXXXXX")"
...
rm -f "$contract_log"
[ "$contract_count" = "287" ] || fail "Expected 287 PHP contracts, got $contract_count."
printf 'PASS - %d PHP contract files executed sequentially\n' "$contract_count"
printf '==> Runtime identity and preserved WordPress slug\n'
php -r '$p=file_get_contents($argv[1]); if(!preg_match("/Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform/",$p)||!preg_match("/Version:[[:space:]]*7\\.7\\.1/",$p)||!preg_match("/const VERSION = '\''7\\.7\\.1'\'';/",$p)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity mismatch."
...
[ "$js_count" = "62" ] || fail "Expected 62 JavaScript files, got $js_count."
...
COUNTS
[ "$json_count" = "284" ] || fail "Expected 284 JSON files, got $json_count."
...
[ "$css_count" = "73" ] || fail "Expected 73 CSS layers, got $css_count."
printf 'PASS - %d balanced CSS layers\n' "$css_count"
printf '==> v7.7.1 Plugin Discovery Intelligence metadata\n'
"$PYTHON_BIN" - "$ROOT" <<'PY'
from pathlib import Path
import json,sys
r=Path(sys.argv[1]);version='7.7.1';name='Plugin Discovery Intelligence'
current=json.loads((r/'feature_suggestions_manifest.json').read_text())
versioned=json.loads((r/'feature_suggestions_manifest-v7.7.1.json').read_text())
release=json.loads((r/'release-manifest-v7.7.1.json').read_text())
intelligence=json.loads((r/'schemas/scfs-plugin-discovery-intelligence-v1.schema.json').read_text())
...
assert current['validation']['php_contract_files']==287
assert current['validation']['php_source_files']==54
assert current['validation']['javascript_files']==62
assert current['validation']['json_files']==284
assert current['validation']['python_files']==76
assert current['validation']['css_layers']==73
assert current['validation']['backend_tests_passed']==322
assert current['installed_plugin_discovery']['version']==version
assert intelligence['properties']['version']['const']==version
cap=current['plugin_discovery_intelligence']
for key in ('complete_plugin_inventory','standard_site_network_inactive_classification','must_use_and_dropin_detection','confidence_ranked_suggestions','matching_signal_explanation','bulk_mapping_and_ignore','duplicate_mapping_detection','installed_vs_github_version_comparison','plugin_header_repository_consistency','search_and_filters','human_confirmation_required','legacy_shortcodes_preserved','accessibility_behavior_preserved'):
 assert cap[key] is True,key
print('PASS - Plugin Discovery Intelligence release metadata')
PY
printf '==> Plugin Discovery Intelligence contracts\n'
php "$ROOT/tests/test-v771-plugin-discovery-intelligence.php" >/dev/null || fail "Plugin Discovery Intelligence source contract failed."
php "$ROOT/tests/test-v771-plugin-discovery-runtime.php" >/dev/null || fail "Plugin Discovery Intelligence runtime contract failed."
printf 'PASS - inventory, classification, confidence, bulk review, duplicate, and version comparison controls\n'
...
[ "$backend_passed" = "322" ] || fail "Expected 322 backend tests, got ${backend_passed:-unknown}."
printf 'PASS - 322 backend tests\n'
