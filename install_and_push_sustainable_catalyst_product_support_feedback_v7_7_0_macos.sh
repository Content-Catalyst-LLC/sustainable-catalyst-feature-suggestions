...
set -Eeuo pipefail
VERSION="7.7.0"
INSTALLER_REVISION="V7_7_0_CANONICAL_PRODUCT_REGISTRY_ADMINISTRATION"
RELEASE_TITLE="Canonical Product Registry Administration"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v7_7_0.sh"
REPO_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.7.0-repository.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v7.7.0-release-bundle.zip"
SUMS_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.7.0-artifacts.sha256"
...
[ -n "$ARCHIVE" ] || fail "Place the v7.7.0 repository ZIP or release bundle beside this installer or in ~/Downloads."
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scpsf-v770.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
if [ -n "$REPO_ZIP" ]; then
  ...
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.7.0-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.7.0-repository' -print -quit)"
fi
[ -n "${SOURCE:-}" ] || fail "Could not locate the v7.7.0 repository source."
...
grep -Fq 'VERSION="7.7.0"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "Wrong validator version."
grep -Fq 'Version: 7.7.0' "$SOURCE/wordpress/$PLUGIN_SLUG/$PLUGIN_SLUG.php" || fail "Wrong plugin version."
...
grep -Fq 'MutationObserver' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-console-v7.7.0.js" || fail "Dynamic Release Console initialization is missing."
grep -Fq '[data-console-active="true"]' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.7.0.css" || fail "Stable Release Console screen layout is missing."
grep -Fq -- '--scfs-release-console-columns:' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.7.0.css" || fail "Shared responsive Release Console grid is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.7.0.js" ] || fail "Plugin Discovery mapping JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.7.0.css" ] || fail "Plugin Discovery mapping stylesheet is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-operations-v7.7.0.js" ] || fail "Release Operations JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-operations-v7.7.0.css" ] || fail "Release Operations stylesheet is missing."
...
grep -Fq "const VERSION = '7.7.0';" "$REGISTRY_ADMIN_CLASS" || fail "Canonical Product Registry Administration version is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/canonical-product-registry-admin-v7.7.0.js" ] || fail "Registry administration JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/canonical-product-registry-admin-v7.7.0.css" ] || fail "Registry administration stylesheet is missing."
...
if [ "${SCPSF_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v7.7.0 Canonical Product Registry Administration package and preserved the WordPress plugin identity.\n'
  exit 0
fi
...
printf '==> Installing v7.7.0 source with checksum verification\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v7.7.0.\n'
else
  git commit -m "Product Support and Feedback Platform v7.7.0 — $RELEASE_TITLE"
fi
...
push_release_tag
printf '\nSUCCESS: v7.7.0 validated, installed, committed, tagged, and pushed.\n'
