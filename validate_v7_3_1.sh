...
contract_log="$(mktemp "${TMPDIR:-/tmp}/scpsf-v731-contract.XXXXXX")"
...
php -r '$plugin=file_get_contents($argv[1]);if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform[[:space:]]*$/m",$plugin)){exit(1);}if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Version:[[:space:]]*7\\.3\\.1[[:space:]]*$/m",$plugin)){exit(1);}if(!preg_match("/const VERSION = '\''7\\.3\\.1'\'';/",$plugin)){exit(1);}if(!preg_match("/^[[:space:]]*\\*[[:space:]]*Text Domain:[[:space:]]*sustainable-catalyst-feature-suggestions[[:space:]]*$/m",$plugin)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity or compatibility check failed."
...
printf '==> Release telemetry manifests and schema\n'
"$PYTHON_BIN" - "$ROOT" <<'PYMANIFEST'
from pathlib import Path
import json,sys
r=Path(sys.argv[1])
current=json.loads((r/'feature_suggestions_manifest.json').read_text())
versioned=json.loads((r/'feature_suggestions_manifest-v7.3.1.json').read_text())
historical=json.loads((r/'feature_suggestions_manifest-v7.2.1.json').read_text())
release=json.loads((r/'release-manifest-v7.3.1.json').read_text())
example=json.loads((r/'examples/release-board-v7.3.1.json').read_text())
schema=json.loads((r/'schemas/scfs-release-board-v1.schema.json').read_text())
assert current==versioned
assert current['version']=='7.3.1'
assert current['release_name']=='Release Telemetry and Homepage Presentation'
assert current['repository']=='Content-Catalyst-LLC/sustainable-catalyst-product-support-feedback'
board=current['release_board']
assert board['schema']=='scfs-release-board/1.1'
...
assert board['public_title']=='Release Telemetry'
...
assert current['canonical_product_registry']['schema']=='scfs-canonical-product-registry/1.1'
assert historical['version']=='7.2.1'
assert release['version']=='7.3.1'
assert release['release_name']=='Release Telemetry and Homepage Presentation'
assert release['compatibility']['database_migration_required'] is False
assert example['schema']==board['schema']
assert example['version']=='7.3.1'
assert schema['properties']['version']['const']=='7.3.1'
print('PASS - Release Telemetry identity, presentation, labels, governance, privacy, accessibility, and compatibility contracts validated')
PYMANIFEST
printf '==> Release telemetry source contract\n'
BOARD_CLASS="$PLUGIN/includes/class-scfs-release-board.php"
BOARD_CSS="$PLUGIN/assets/release-board-v7.3.1.css"
[ -f "$BOARD_CLASS" ] || fail "Missing release board class."
[ -f "$BOARD_CSS" ] || fail "Missing release board stylesheet."
grep -Fq "const VERSION = '7.3.1';" "$BOARD_CLASS" || fail "Release board version mismatch."
...
for attribute in layout context groups products limit show_status show_updated show_links show_header show_footer show_source dense inactive title heading_level; do grep -Fq "'$attribute'" "$BOARD_CLASS" || fail "Shortcode attribute missing: $attribute"; done
...
grep -Fq "__('Release Telemetry'" "$BOARD_CLASS" || fail "Release Telemetry title missing."
...
INSTALLER="$ROOT/install_and_push_sustainable_catalyst_product_support_feedback_v7_3_1_macos.sh"
[ -f "$INSTALLER" ] || fail "Missing v7.3.1 installer."
...
if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$INSTALLER" "$ROOT/validate_v7_3_1.sh"; then fail "Release scripts require Bash 4."; fi
