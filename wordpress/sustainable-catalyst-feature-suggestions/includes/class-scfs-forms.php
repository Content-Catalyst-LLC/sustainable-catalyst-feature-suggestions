...
final class SCFS_Forms_Foundation {
    ...
    public function register_assets() {
        wp_register_style('scfs-forms', plugins_url('../assets/forms.css', __FILE__), array(), '2.4.0');
        wp_register_script('scfs-forms', plugins_url('../assets/forms.js', __FILE__), array(), '2.4.0', true);
    }

    public function admin_assets($hook) {
        $screen = function_exists('get_current_screen') ? get_current_screen() : null;
        if (!$screen || $screen->post_type !== self::FORM_TYPE) { return; }
        wp_enqueue_style('scfs-forms-admin', plugins_url('../assets/forms-admin.css', __FILE__), array(), '2.4.0');
        wp_enqueue_script('scfs-forms-admin', plugins_url('../assets/forms-admin.js', __FILE__), array(), '2.4.0', true);
    }
    ...
    private function field_row($i,$field) {
        $f=wp_parse_args($field,array('key'=>'','label'=>'','type'=>'text','required'=>false,'help'=>'','placeholder'=>'','options'=>array(),'validation'=>array()));
    ...
    public function save_form($post_id,$post) {
        ...
        foreach($raw as $row){
            ...
            $fields[]=array('key'=>$key,'label'=>$label,'type'=>$type,'required'=>!empty($row['required']),'help'=>sanitize_textarea_field($row['help']??''),'placeholder'=>sanitize_text_field($row['placeholder']??''),'options'=>$opts);
        }
    ...
    public function render_shortcode($atts) {
        ...
        $schema=get_post_meta($form->ID,'_scfs_form_schema',true); $fields=$schema['fields']??array();
        ...
        <section class="scfs-dynamic-form <?php echo esc_attr($atts['class']); ?>" aria-labelledby="scfs-form-title-<?php echo esc_attr($form->ID); ?>">
          ...
          <form method="post" class="scfs-form" novalidate>
            ...
            <?php foreach($fields as $field) $this->render_field($field); ?>
    ...
    private function render_field($f) {
        ...
        echo '<div class="scfs-form-field scfs-type-'.esc_attr($f['type']).'"><label for="'.esc_attr($id).'">'.esc_html($f['label']).($required?' <span aria-hidden="true">*</span>':'').'</label>';
    ...
    public function handle_submission() {
        ...
        $schema=get_post_meta($form_id,'_scfs_form_schema',true); $input=isset($_POST['scfs_response'])&&is_array($_POST['scfs_response'])?wp_unslash($_POST['scfs_response']):array(); $answers=array(); $errors=array();
        foreach(($schema['fields']??array()) as $f){$raw=$input[$f['key']]??''; $value=$this->sanitize_answer($raw,$f); if(!empty($f['required'])&&($value===''||$value===array()))$errors[]=$f['key']; $answers[$f['key']]=$value;}
        ...
        do_action('scfs_event',array('event_id'=>wp_generate_uuid4(),'event_type'=>'form.response_submitted','schema_version'=>'1.0','source'=>'feature_suggestions','source_version'=>'2.4.0','occurred_at'=>gmdate('c'),'data'=>array('response_uuid'=>$uuid,'form_id'=>$form_id,'form_slug'=>$form->post_name,'instrument_type'=>get_post_meta($form_id,'_scfs_form_mode',true),'field_count'=>count($answers))));
