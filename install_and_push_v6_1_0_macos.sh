...
VERSION="6.1.0"
INSTALLER_REVISION="V6_1_0_REMOTE_SYNC_SAFE"
RELEASE_TITLE="Help Desk Case Foundation"
...
ARCHIVE=""
for candidate in \
  "$SCRIPT_DIR/sustainable-catalyst-feature-suggestions-v6.1.0-repo.zip" \
  "$DOWNLOADS/sustainable-catalyst-feature-suggestions-v6.1.0-repo.zip" \
  "$SCRIPT_DIR/sustainable-catalyst-product-support-feedback-platform-v6.1.0-release-bundle.zip" \
  "$DOWNLOADS/sustainable-catalyst-product-support-feedback-platform-v6.1.0-release-bundle.zip"; do
  if [ -f "$candidate" ]; then
    ARCHIVE="$candidate"
    break
  fi
done
[ -n "$ARCHIVE" ] || fail "Place the v6.1.0 repository ZIP or release bundle beside this installer or in ~/Downloads."

TMP="$(mktemp -d "${TMPDIR:-/tmp}/scfs-v610.XXXXXX")"
cleanup() { rm -rf "$TMP"; }
trap cleanup EXIT
...
REPO_ZIP="$(find "$TMP/archive" -type f -name 'sustainable-catalyst-feature-suggestions-v6.1.0-repo.zip' -print -quit || true)"
if [ -n "$REPO_ZIP" ]; then
  mkdir -p "$TMP/repository"
  unzip -q "$REPO_ZIP" -d "$TMP/repository"
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-feature-suggestions-v6.1.0-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-feature-suggestions-v6.1.0-repository' -print -quit)"
fi

[ -n "${SOURCE:-}" ] || fail "Could not locate the v6.1.0 repository source."
[ -f "$SOURCE/validate_v6_1_0.sh" ] || fail "The selected archive does not contain validate_v6_1_0.sh."
[ -f "$SOURCE/backend/requirements-validation.txt" ] || fail "The selected archive is missing backend/requirements-validation.txt."

if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$SOURCE/validate_v6_1_0.sh"; then
  fail "The packaged validation script requires Bash 4. Use the portable v6.1.0 release."
fi
bash -n "$SOURCE/validate_v6_1_0.sh" || fail "The packaged validation script has invalid shell syntax."

if [ "${SCFS_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected a complete v6.1.0 source archive.\n'
  exit 0
fi
...
PYTHON_BIN="$VENV_PY" bash "$SOURCE/validate_v6_1_0.sh"

if [ -d "$REPO_DIR/.git" ]; then
  TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
  BACKUP="$DOWNLOADS/sustainable-catalyst-feature-suggestions-before-v6.1.0-${TIMESTAMP}.zip"
  GIT_BUNDLE="$DOWNLOADS/sustainable-catalyst-feature-suggestions-before-v6.1.0-${TIMESTAMP}.bundle"
...
printf '==> Installing v6.1.0 source onto current origin/main\n'
rsync -a --delete --exclude '.git/' --exclude '.venv/' --exclude 'venv/' "$SOURCE/" "$REPO_DIR/"
cd "$REPO_DIR"

PYTHON_BIN="$VENV_PY" bash ./validate_v6_1_0.sh
git diff --check
git add -A

if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v6.1.0.\n'
else
  git commit -m "Product Support and Feedback Platform v6.1.0 — $RELEASE_TITLE"
fi
...
printf '\nSUCCESS: v6.1.0 synchronized with current origin/main, validated, committed, tagged, and pushed.\n'
