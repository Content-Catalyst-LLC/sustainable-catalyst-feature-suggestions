...
files=list(root.glob('sustainable-catalyst-feature-suggestions-v5.2.2-repo*.zip'))+list(root.glob('sustainable-catalyst-feature-suggestions-v5.2.2-release-bundle*.zip'))
...
for f in "$MAIN" "$KB" "$CSS" tests/test-v522-shortcode-separation.php RELEASE_NOTES_5.2.2.md; do [ -f "$f" ] || { echo "ERROR: Missing $f"; exit 1; }; done
grep -Fq 'Version: 5.2.2' "$MAIN" || { echo "ERROR: WordPress version marker mismatch."; exit 1; }
grep -Fq "const VERSION = '5.2.2';" "$KB" || { echo "ERROR: Knowledge Base version marker mismatch."; exit 1; }
