...
if ! printf '%s\n' "$LAYOUT_OUTPUT" | grep -Fq 'PASS - 11 Knowledge Base article header checks'; then
  echo "ERROR: The v5.2.2 publication-parity and expanded-default contract did not pass all 11 checks."
  exit 1
fi
...
if ! grep -Fq "const VERSION = '5.2.2';" "$KB_ZIP_FILE"; then
  echo "ERROR: WordPress distribution ZIP does not contain the v5.2.2 full-width Knowledge Base class."
  exit 1
fi
