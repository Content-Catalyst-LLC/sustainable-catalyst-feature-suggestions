...
set -Eeuo pipefail
VERSION="7.6.2"
INSTALLER_REVISION="V7_6_2_PRODUCT_CONNECTION_EDITOR"
RELEASE_TITLE="Product Connection Editor"
...
TAG="v${VERSION}"
VALIDATOR_SCRIPT="validate_v7_6_2.sh"
REPO_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.6.2-repository.zip"
BUNDLE_ARCHIVE="sustainable-catalyst-product-support-feedback-platform-v7.6.2-release-bundle.zip"
SUMS_ARCHIVE="sustainable-catalyst-product-support-feedback-v7.6.2-artifacts.sha256"
...
[ -n "$ARCHIVE" ] || fail "Place the v7.6.2 repository ZIP or release bundle beside this installer or in ~/Downloads."
TMP="$(mktemp -d "${TMPDIR:-/tmp}/scpsf-v762.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
if [ -n "$REPO_ZIP" ]; then
  ...
  SOURCE="$(find "$TMP/repository" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.6.2-repository' -print -quit)"
else
  SOURCE="$(find "$TMP/archive" -maxdepth 2 -type d -name 'sustainable-catalyst-product-support-feedback-v7.6.2-repository' -print -quit)"
fi
[ -n "${SOURCE:-}" ] || fail "Could not locate the v7.6.2 repository source."
...
grep -Fq 'VERSION="7.6.2"' "$SOURCE/$VALIDATOR_SCRIPT" || fail "Wrong validator version."
grep -Fq 'Version: 7.6.2' "$SOURCE/wordpress/$PLUGIN_SLUG/$PLUGIN_SLUG.php" || fail "Wrong plugin version."
...
grep -Fq 'MutationObserver' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-console-v7.6.2.js" || fail "Dynamic Release Console initialization is missing."
grep -Fq '[data-console-active="true"]' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.6.2.css" || fail "Stable Release Console screen layout is missing."
grep -Fq -- '--scfs-release-console-columns:' "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-board-v7.6.2.css" || fail "Shared responsive Release Console grid is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.6.2.js" ] || fail "Plugin Discovery mapping JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/plugin-discovery-v7.6.2.css" ] || fail "Plugin Discovery mapping stylesheet is missing."
...
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-operations-v7.6.2.js" ] || fail "Release Operations JavaScript is missing."
[ -f "$SOURCE/wordpress/$PLUGIN_SLUG/assets/release-operations-v7.6.2.css" ] || fail "Release Operations stylesheet is missing."
...
if [ "${SCPSF_PREFLIGHT_ONLY:-0}" = "1" ]; then
  printf 'PREFLIGHT PASSED: selected the v7.6.2 Product Connection Editor package and preserved the WordPress plugin identity.\n'
  exit 0
fi
...
printf '==> Installing v7.6.2 source with checksum verification\n'
...
if git diff --cached --quiet; then
  printf 'No source changes to commit. Current main may already contain v7.6.2.\n'
else
  git commit -m "Product Support and Feedback Platform v7.6.2 — $RELEASE_TITLE"
fi
...
push_release_tag
printf '\nSUCCESS: v7.6.2 validated, installed, committed, tagged, and pushed.\n'
