...
VERSION="5.2.9"
INSTALLER_REVISION="V5_2_9_R1_SUPPORT_DISCOVERY"
RELEASE_NAME="Support Discovery, Navigation, and Search Quality"
REPOSITORY="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-feature-suggestions.git"
DOWNLOADS_DIR="${HOME}/Downloads"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/scfs-v529.XXXXXX")"
...
[ -n "$PYTHON_BIN" ] || fail 'Python 3.12 or 3.13 is required for v5.2.9 validation. Install one with: brew install python@3.13'
...
patterns = (
    'sustainable-catalyst-product-support-feedback-platform-v5.2.9-release-bundle*.zip',
    'sustainable-catalyst-feature-suggestions-v5.2.9-repo*.zip',
)
files = []
...
[ -f "$SOURCE_ARCHIVE" ] || fail 'No v5.2.9 repository ZIP or release bundle was found in ~/Downloads.'
printf 'Using release archive: %s\n' "$SOURCE_ARCHIVE"
unzip -q "$SOURCE_ARCHIVE" -d "$STAGE_DIR"

INNER_ZIP="$(find "$STAGE_DIR" -maxdepth 8 -type f -name 'sustainable-catalyst-feature-suggestions-v5.2.9-repo*.zip' -print -quit)"
if [ -n "$INNER_ZIP" ]; then
...
discovery = manifest.get('support_discovery', {})
assert discovery.get('schema') == 'scfs-support-discovery/1.0'
assert discovery.get('personal_data_stored') is False
PY
...
for required in \
  ...
  wordpress/sustainable-catalyst-feature-suggestions/includes/class-scfs-support-article-integrity.php \
  wordpress/sustainable-catalyst-feature-suggestions/includes/class-scfs-support-discovery.php \
  wordpress/sustainable-catalyst-feature-suggestions/assets/support-article-integrity.css \
  backend/app/support_article_integrity.py \
  backend/tests/test_support_article_integrity.py \
  backend/app/support_discovery.py \
  backend/tests/test_support_discovery.py \
  tests/test-v529-bootstrap.php \
  tests/test-v529-search-quality.php \
  tests/test-v529-interface.php \
  tests/test-v529-api-and-compatibility.php \
  tests/test-v529-release-artifacts.php \
  docs/support-discovery-navigation-search-v5.2.9.md \
  examples/support-discovery-search.json \
  schemas/scfs-support-discovery-v1.schema.json \
  RELEASE_NOTES_5.2.9.md \
  release-manifest-v5.2.9.json \
  validate_v5_2_9.sh; do
...
PYTHON_BIN="$VENV_DIR/bin/python" ./validate_v5_2_9.sh
...
printf 'Support discovery: https://sustainablecatalyst.com/support/?scfs_support_view=documentation#knowledge-base\n'
