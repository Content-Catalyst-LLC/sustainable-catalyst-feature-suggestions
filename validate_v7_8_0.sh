...
contract_log="$(mktemp "${TMPDIR:-/tmp}/scpsf-v780-contract.XXXXXX")"
...
rm -f "$contract_log"
[ "$contract_count" = "289" ] || fail "Expected 289 PHP contracts, got $contract_count."
printf 'PASS - %d PHP contract files executed sequentially\n' "$contract_count"
printf '==> Runtime identity and preserved WordPress slug\n'
php -r '$p=file_get_contents($argv[1]); if(!preg_match("/Plugin Name:[[:space:]]*Sustainable Catalyst Product Support and Feedback Platform/",$p)||!preg_match("/Version:[[:space:]]*7\\.8\\.0/",$p)||!preg_match("/const VERSION = '\''7\\.8\\.0'\'';/",$p)){exit(1);}' "$PLUGIN/sustainable-catalyst-feature-suggestions.php" || fail "Plugin identity mismatch."
...
[ "$js_count" = "68" ] || fail "Expected 68 JavaScript files, got $js_count."
...
COUNTS
[ "$json_count" = "298" ] || fail "Expected 298 JSON files, got $json_count."
...
[ "$css_count" = "79" ] || fail "Expected 79 CSS layers, got $css_count."
printf 'PASS - %d balanced CSS layers\n' "$css_count"
printf '==> v7.8.0 GitHub Release Intelligence metadata\n'
"$PYTHON_BIN" - "$ROOT" <<'PY'
from pathlib import Path
import json,sys
r=Path(sys.argv[1]);version='7.8.0';name='GitHub Release Intelligence'
current=json.loads((r/'feature_suggestions_manifest.json').read_text())
versioned=json.loads((r/'feature_suggestions_manifest-v7.8.0.json').read_text())
release=json.loads((r/'release-manifest-v7.8.0.json').read_text())
...
assert current['validation']['php_contract_files']==289
assert current['validation']['php_source_files']==55
assert current['validation']['javascript_files']==68
assert current['validation']['json_files']==298
assert current['validation']['python_files']==78
assert current['validation']['css_layers']==79
...
print('PASS - GitHub Release Intelligence release metadata')
PY
printf '==> GitHub Release Intelligence contracts\n'
php "$ROOT/tests/test-v780-github-release-intelligence.php" >/dev/null || fail "GitHub Release Intelligence source contract failed."
php "$ROOT/tests/test-v780-github-release-intelligence-runtime.php" >/dev/null || fail "GitHub Release Intelligence runtime contract failed."
...
if grep -Eq '(^|[[:space:]])(mapfile|readarray)([[:space:]]|$)' "$ROOT/validate_v7_8_0.sh"; then fail "Validator requires Bash 4."; fi
bash -n "$ROOT/validate_v7_8_0.sh" || fail "Validator shell syntax failed."
